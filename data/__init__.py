...
from data.MTLCC.data_transforms_transfer import MTLCC_transform_transfer
...
from data.France.data_transforms_transfer import France_segmentation_transform_transfer
...
from data.PASTIS24.data_transforms_transfer import PASTIS_segmentation_transform_transfer
    ...
    if config['TRANSFER']:
        if train_config['dataset'] == 'MTLCC':
            dataloaders['train'] = get_mtlcc_dataloader(
                ...
                transform=MTLCC_transform_transfer(model_config, train_config, is_training=True),
            ...
            dataloaders['train'] = get_pastis_dataloader(
                paths_file=train_config['paths'], root_dir=train_config['base_dir'],
                transform=PASTIS_segmentation_transform_transfer(model_config, is_training=True),
                batch_size=train_config['batch_size'], shuffle=True, num_workers=train_config['num_workers'])
        else:
            dataloaders['train'] = get_france_dataloader(
                paths_file=train_config['paths'], root_dir=train_config['base_dir'],
                transform=France_segmentation_transform_transfer(model_config, train_config, is_training=True),
                batch_size=train_config['batch_size'], shuffle=True, num_workers=train_config['num_workers'])
    else:
        if train_config['dataset'] == 'MTLCC':
            dataloaders['train'] = get_mtlcc_dataloader(
    ...
    if config['TRANSFER']:
        if eval_config['dataset'] == 'MTLCC':
            dataloaders['eval'] = get_mtlcc_dataloader(
                ...
                transform=MTLCC_transform_transfer(model_config, eval_config, is_training=False),
            ...
            dataloaders['eval'] = get_pastis_dataloader(
                paths_file=eval_config['paths'], root_dir=eval_config['base_dir'],
                transform=PASTIS_segmentation_transform_transfer(model_config, is_training=False),
                batch_size=eval_config['batch_size'], shuffle=False, num_workers=eval_config['num_workers'])
        else:
            dataloaders['eval'] = get_france_dataloader(
                paths_file=eval_config['paths'], root_dir=eval_config['base_dir'],
                transform=France_segmentation_transform_transfer(model_config, eval_config, is_training=True),
                batch_size=eval_config['batch_size'], shuffle=False, num_workers=eval_config['num_workers'])
    else:
        if eval_config['dataset'] == 'MTLCC':
            dataloaders['eval'] = get_mtlcc_dataloader(
