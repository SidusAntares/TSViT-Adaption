...
class TSViT_extract(nn.Module):
    ...
    def forward(self, x):
        ...
        xt = x[:, :, -1, 0, 0]
        x = x[:, :, :-1] #torch.Size([1, 60, 10, 24, 24])
        ...
        # self.to_patch_embedding = nn.Sequential(
        #     Rearrange('b t c (h p1) (w p2) -> (b h w) t (p1 p2 c)', p1=self.patch_size, p2=self.patch_size),
        #     nn.Linear(patch_dim, self.dim),)   patch_dim=40
        
        x = self.to_patch_embedding(x) #torch.Size([144, 60, 128])
        x = x.reshape(B, -1, T, self.dim) #torch.Size([1, 144, 60, 128])
        x += temporal_pos_embedding.unsqueeze(1)
        x = x.reshape(-1, T, self.dim) # torch.Size([144, 60, 128])
        cls_temporal_tokens = repeat(self.temporal_token, '() N d -> b N d', b=B * self.num_patches_1d ** 2) #torch.Size([144, 6, 128])
        x = torch.cat((cls_temporal_tokens, x), dim=1)
        x = self.temporal_transformer(x)
        x = x[:, :self.num_classes]

        feature = x #torch.Size([144, 6, 128])

        feature =feature.reshape(B, (H//self.patch_size) , (W//self.patch_size), self.num_classes, self.dim)
        # print('feature.shape: ', feature.shape)
        h_idxs = torch.arange(H, device=feature.device).view(-1, 1).expand(H, W)
        w_idxs = torch.arange(W, device=feature.device).view(1, -1).expand(H, W)
        patch_i = h_idxs // self.patch_size  # [H, W] -> [24, 24]
        patch_j = w_idxs // self.patch_size  # [H, W] -> [24, 24]
        feature = feature[:,patch_i, patch_j, :, :] #torch.Size([1, 24, 24, 6, 128])
        ...
        #torch.Size([6, 144, 128])
        x += self.space_pos_embedding#[:, :, :(n + 1)]
        x = self.dropout(x)
        x = self.space_transformer(x)# torch.Size([6, 144, 128])
        x = self.mlp_head(x.reshape(-1, self.dim)) #torch.Size([864, 4])
        x = x.reshape(B, self.num_classes, self.num_patches_1d**2, self.patch_size**2).permute(0, 2, 3, 1) #torch.Size([1, 144, 4, 6])
        x = x.reshape(B, H, W, self.num_classes) #torch.Size([1, 24, 24, 6])
        x = x.permute(0, 3, 1, 2) 
        return feature,x
